...
public class GUI extends JFrame {
	...
	public GUI() {
		...
		JScrollPane sc4 = new JScrollPane(d, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		JScrollPane sc3 = new JScrollPane(c, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		JScrollPane sc2 = new JScrollPane(b, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		log = new DefaultListModel<>();
		displayy = new DefaultListModel<>();
		JList<String> f = new JList<>(displayy);
		displayPanel = new JScrollPane(f, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		displayPanel.setPreferredSize(new Dimension(200, 120));
		left.add(sc2, BorderLayout.NORTH);
		JList<String> a = new JList<>(log);
		JScrollPane sc = new JScrollPane(a, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	public void updateDisasters(CommandCenter c) {
		...
		for (int i = 0; i < c.getVisibleBuildings().size(); i++) {
			if (c.getVisibleBuildings().get(i).getDisaster().isActive()
					&& c.getVisibleBuildings().get(i).getStructuralIntegrity() != 0) {
				disasters.addElement(
						c.getVisibleBuildings().get(i).getDisaster() + " on " + c.getVisibleBuildings().get(i));
			}
		}
		for (int i = 0; i < c.getVisibleCitizens().size(); i++) {
			if (c.getVisibleCitizens().get(i).getDisaster().isActive() && c.getVisibleCitizens().get(i).getHp() != 0) {
				disasters.addElement(c.getVisibleCitizens().get(i).getDisaster() + " on "
						+ c.getVisibleCitizens().get(i) + " at " + c.getVisibleCitizens().get(i).getLocation().getX()
						+ ", " + c.getVisibleCitizens().get(i).getLocation().getY());
			}
		}
	...
	public void updateUnitInfo(Unit u) {
		...
		if (u.getTarget() instanceof Citizen)
			unitInfo.addElement("Target: Citizen at location " + u.getTarget().getLocation().getX() + ","
					+ u.getTarget().getLocation().getY());
		if (u.getTarget() instanceof ResidentialBuilding)
			unitInfo.addElement("Target: Building at location " + u.getTarget().getLocation().getX() + ","
					+ u.getTarget().getLocation().getY());
		if (u instanceof Evacuator) {
	...
	public void updateLog(Simulator s) {
		...
		for (int i = 0; i < s.getExecutedDisasters().size(); i++) {
			if (s.getExecutedDisasters().get(i).getStartCycle() == s.getCurrentCycle()) {
				log.addElement("        " + "A " + s.getExecutedDisasters().get(i) + " disaster has struck "
						+ s.getExecutedDisasters().get(i).getTarget() + ".");
			}
		}
		...
		for (int i = 0; i < s.getCitizens().size(); i++) {
			if (s.getCitizens().get(i).getState() == CitizenState.DECEASED && s.getCitizens().get(i).isJustDied()) {
				log.addElement("        " + s.getCitizens().get(i) + " at location:  "
						+ s.getCitizens().get(i).getLocation().getX() + ", "
						+ s.getCitizens().get(i).getLocation().getY());
	...
	public void updateInfo(Simulator s, CommandCenter c, int x, int y) {
		...
		for (int i = 0; i < c.getEmergencyUnits().size(); i++) {
			if (c.getEmergencyUnits().get(i).getLocation().getX() == x
					&& c.getEmergencyUnits().get(i).getLocation().getY() == y) {
			...
			if (c.getEmergencyUnits().get(i).getLocation().equals(temp)) {
				...
				if (c.getEmergencyUnits().get(i).getTarget() != null) {
					if (c.getEmergencyUnits().get(i).getTarget() instanceof Citizen) {
						InfoPanel.addElement(
								"Target: Citizen at " + c.getEmergencyUnits().get(i).getTarget().getLocation().getX()
										+ ", " + c.getEmergencyUnits().get(i).getTarget().getLocation().getY());
					} else {
						InfoPanel.addElement(
								"Target: Building at " + c.getEmergencyUnits().get(i).getTarget().getLocation().getX()
										+ ", " + c.getEmergencyUnits().get(i).getTarget().getLocation().getY());
					}
				}
